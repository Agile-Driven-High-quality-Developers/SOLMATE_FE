    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-90 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800">
          <p className="text-[15px] font-bold text-gray-900 dark:text-gray-100">회원탈퇴</p>
        ...
        <div className="px-6 py-6 flex flex-col gap-4">
          {isGoogle ? (
            <div>
              <p className="text-[14px] text-gray-700 dark:text-gray-300 font-medium mb-1">정말 탈퇴하시겠어요?</p>
              <p className="text-[13px] text-gray-400 dark:text-slate-500">탈퇴 후 계정 정보는 영구적으로 삭제돼요.</p>
            </div>
          ) : (
            <div>
              <p className="text-[14px] text-gray-700 dark:text-gray-300 font-medium mb-1">탈퇴 전 본인 확인이 필요해요</p>
              <p className="text-[13px] text-gray-400 dark:text-slate-500">비밀번호를 입력하면 계정이 영구적으로 삭제돼요.</p>
            </div>
          )}
